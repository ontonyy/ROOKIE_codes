from my_modules.URSINA.shit_snake.game_objects import *
...
class Game(Ursina):
    ...
    def create_map(self, MAP_SIZE):
        Entity(model='quad', scawle=MAP_SIZE, position=(MAP_SIZE // 2, MAP_SIZE // 2, 0), color=color.dark_gray)
    ...
    def check_game_over(self):
        ...
        if 0 < snake[-1][0] < self.MAP_SIZE and 0 < snake[-1][1] < self.MAP_SIZE and len(snake) == len(set(snake)):
            return
        print_on_screen("GAME OVER!", position=(-0.015, 0.02), scale=.2, duration=1)
